...
// [GET] Obtener productos con búsqueda y filtros (aplica al presionar "Buscar")
router.get('/', async (req, res) => {
  try {
    const { search, minPrice, maxPrice, condition } = req.query;
    let query = { status: 'available' };

    // Añadir filtros de búsqueda en título, descripción y tags
    if (search) {
      query.$or = [
    ...
    // Aplicar filtros de precio y condición si se proporcionan
    ...
    // Ordenar los productos por relevancia en función de la búsqueda y los filtros
    products = products.sort((a, b) => {
      ...
      if (search) {
        if (searchRegex.test(a.title) || searchRegex.test(a.description) || a.tags.includes(search)) aScore += 2;
        if (searchRegex.test(b.title) || searchRegex.test(b.description) || b.tags.includes(search)) bScore += 2;
      }
